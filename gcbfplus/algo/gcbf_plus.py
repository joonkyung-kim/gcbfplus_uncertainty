...
from gcbfplus.trainer.utils import (
...
class GCBFPlus(GCBF):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cbf_train_state = TrainState.create(
            apply_fn=self.cbf.get_cbf, params=cbf_params, tx=self.cbf_optim
        )
        self.cbf_tgt = TrainState.create(
            apply_fn=self.cbf.get_cbf, params=tree_copy(cbf_params), tx=empty_grad_tx()
        )
        ...
        self.actor_train_state = TrainState.create(
            apply_fn=self.actor.sample_action, params=actor_params, tx=self.actor_optim
        )
        ...
        def safe_rollout(single_rollout_mask: Array) -> Array:
            ...
            for i in range(single_rollout_mask.shape[0]):
                ...
                safe_rollout_mask = safe_rollout_mask.at[start : i + 1].set(
                    (
                        (1 - single_rollout_mask[i]) * safe_rollout_mask[start : i + 1]
                    ).astype(jnp.bool_)
                )
    ...
    def update_tgt(
        self, cbf_tgt: TrainState, cbf: TrainState, tau: float
    ) -> TrainState:
    ...
    def get_b_u_qp(self, b_graph: GraphsTuple, params) -> Action:
        b_u_qp, bT_relaxation = jax_vmap(
            ft.partial(self.get_qp_action, cbf_params=params)
        )(b_graph)
        return b_u_qp
    ...
    def update_nets(self, rollout: Rollout, safe_mask, unsafe_mask):
        ...
        for i_epoch in range(self.inner_epoch):
            ...
            # (n_mb, mb_size)
            batch_idx = np.stack(
                np.array_split(idx, idx.shape[0] // self.batch_size), axis=0
            )
    ...
    def sample_batch(self, rollout: Rollout, safe_mask, unsafe_mask):
        if self.buffer.length > self.batch_size:
            # sample from memory
            memory, safe_mask_memory, unsafe_mask_memory = self.buffer.sample(
                rollout.length
            )
            try:
                unsafe_memory, safe_mask_unsafe_memory, unsafe_mask_unsafe_memory = (
    ...
    def update(self, rollout: Rollout, step: int) -> dict:
        ...
        del rollout
        rollout_batch, safe_mask_batch, unsafe_mask_batch = self.sample_batch(
            rollout_np, safe_mask, unsafe_mask
        )

        # inner loop
        update_info = self.update_nets(
            rollout_batch, safe_mask_batch, unsafe_mask_batch
        )
    ...
    def get_qp_action(
        ...
        Lg_h = ei.einsum(
            h_x, dyn_g, "agent_i agent_j nx, agent_j nx nu -> agent_i agent_j nu"
        )
        ...
        H = jnp.eye(
            self._env.action_dim * self.n_agents + self.n_agents, dtype=jnp.float32
        )
        H = H.at[-self.n_agents :, -self.n_agents :].set(
            H[-self.n_agents :, -self.n_agents :] * 10.0
        )
        ...
        r_lb = jnp.array([0.0] * self.n_agents, dtype=jnp.float32)
    ...
    def update_inner(
        ...
        def update_fn(carry, minibatch: Batch):
            ...
            def get_loss(
                cbf_params: Params, actor_params: Params
            ) -> Tuple[Array, dict]:
                # get CBF values
                cbf_fn = jax_vmap(ft.partial(self.cbf.get_cbf, cbf_params))
                cbf_fn_no_grad = jax_vmap(
                ...
                h_unsafe = jnp.where(
                    unsafe_mask_batch, h, -jnp.ones_like(h) * self.eps * 2
                )
                max_val_unsafe = jax.nn.relu(h_unsafe + self.eps)
                loss_unsafe = jnp.sum(max_val_unsafe) / (
                ...
                loss_safe = jnp.sum(max_val_safe) / (
                    jnp.count_nonzero(safe_mask_batch) + 1e-6
                )
                acc_safe_mask = jnp.where(safe_mask_batch, h, -jnp.ones_like(h))
                acc_safe = (jnp.sum(jnp.greater(acc_safe_mask, 0)) + 1e-6) / (
                    jnp.count_nonzero(safe_mask_batch) + 1e-6
                )
                ...
                max_val_h_dot_no_grad = jax.nn.relu(
                    -h_dot_no_grad - self.alpha * h + self.eps
                )
                max_val_h_dot = jnp.where(
                    labeled_mask, max_val_h_dot, max_val_h_dot_no_grad
                )
            ...
            )(cbf.params, actor.params)
            grad_cbf, grad_cbf_norm = compute_norm_and_clip(
                grad_cbf, self.max_grad_norm
            )
            grad_actor, grad_actor_norm = compute_norm_and_clip(
                grad_actor, self.max_grad_norm
            )
            cbf = cbf.apply_gradients(grads=grad_cbf)
            actor = actor.apply_gradients(grads=grad_actor)
            grad_info = {
                "grad_norm/cbf": grad_cbf_norm,
                "grad_norm/actor": grad_actor_norm,
            }
            return (cbf, actor), grad_info | loss_info

        train_state = (cbf_train_state, actor_train_state)
        (cbf_train_state, actor_train_state), info = lax.scan(
            update_fn, train_state, batch
        )
